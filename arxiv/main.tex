...
\begin{table}[h]\centering\footnotesize
\begin{tabular}{>{\raggedright\arraybackslash}p{0.307\textwidth}>{\raggedright\arraybackslash}p{0.307\textwidth}>{\raggedright\arraybackslash}p{0.307\textwidth}}\hline
\textbf{Validation target} & \textbf{Minimum evidence} & \textbf{Stronger evidence} \\\hline
Implementability & Reference workflow-engine demo shows run-oriented UI and protocol concepts & Runnable reference host with published commands and fixtures \\\hline
Event-log fidelity & Sample observed run events and terminal state & Replayable event log with schema validation \\\hline
Interrupt/resume semantics & Screenshot or transcript of pause/resume flow & Automated test that verifies deterministic resume behavior \\\hline
Conformance & Public leaderboard row and scenario results & Independent host passing core profile scenarios \\\hline
Portability & Host capability comparison & Same workflow executed on two compliant hosts with equivalent artifacts \\\hline
Integration reduction & Qualitative adapter mapping & Before/after code or adapter-count comparison \\\hline
\end{tabular}\end{table}
...
OpenWOP complements A2A and MCP & Scope-supported claim & Keep secondary; cite comparison document \\\hline
OpenWOP is implementable in practice & Reference-implementation- and run-capture-supported & Captured live HostCapabilityDocument, conformance snapshot, and a real reference-host run-event log \\\hline
OpenWOP conformance can be evaluated & Conformance-page-supported claim & Add snapshot of host row, profile, scenario results \\\hline
OpenWOP improves portability & Demonstrated for core workflows across two language implementations (Cell A) & Extend to interrupt/replay (Cell B, durable hosts) and to an independent non-steward host \\\hline
