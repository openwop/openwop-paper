...
\title{OpenWOP: A Vendor-Neutral Protocol for Durable, Portable Agentic Workflow Orchestration}
...
\node[draw,rounded corners,fill=black!3,inner sep=3pt,below=18mm of run] (intr) {paused};
...
\draw[->] (run) -- (canc);
\draw[->,transform canvas={xshift=-2mm}] (run) -- node[left,font=\scriptsize]{run.paused} (intr);
...
Workflow-engine reference/demo app & Implementability and practical representation of the protocol & Production-deployment maturity or broad adoption \\\hline
...
TypeScript + Python host result & Cross-language core portability (identical terminal state + event-log structure) & Third-party (non-steward) portability \\\hline
Interrupt/resume capture (durable host) & HITL lifecycle implementability (suspend, correlation-validated resume, replay-debug) & General workflow coverage \\\hline
...
\node[draw,inner sep=3pt,fill=black!3,right=0mm of e3] (e4) {e4: run.paused};
...
\section*{12. Reproducibility and Artifact Availability}
An evidence artifact accompanies this paper. It contains the captured live HostCapabilityDocument; a conformance-leaderboard snapshot (host rows, advertised profiles, suite version, and pass rates); a reference-host run-event log and a durable-host interrupt/resume plus replay-debug capture (suspend $\to$ correlation-validated external resume $\to$ \texttt{run.completed}, including the HTTP~422 mismatch path); the raw cross-host portability captures and experiment design (3/3 core workflows yield identical terminal state and event-log structure across an independent TypeScript and Python host, plus a capability-rejection divergence finding); host-setup instructions; and the 40-entry bibliography, each entry verified against an authoritative source. Two items remain open and are reported as such: independent external-reviewer feedback (a reviewer packet is prepared), and a cross-host \emph{durable} portability comparison together with deterministic fork-from-checkpoint replay.
...
\bibitem[13]{ref13} OASIS. Web Services Business Process Execution Language (WS-BPEL) v2.0. OASIS Standard, 11 April 2007. \url{http://docs.oasis-open.org/wsbpel/2.0/OS/wsbpel-v2.0-OS.html}.
\bibitem[14]{ref14} WfMC. XML Process Definition Language (XPDL) 2.2 (Document WFMC-TC-1025). Workflow Management Coalition, 2012.
...
\bibitem[19]{ref19} LangGraph. \url{https://docs.langchain.com/oss/python/langgraph/overview}.
\bibitem[20]{ref20} Microsoft AutoGen. \url{https://microsoft.github.io/autogen/stable/}.
...
\bibitem[29]{ref29} A Survey on LLM-based Multi-Agent System: Recent Advances and New Frontiers in Application. arXiv:2412.17481, 2024.
...
\bibitem[33]{ref33} Rethinking the Reliability of Multi-agent System: A Perspective from Byzantine Fault Tolerance. arXiv:2511.10400, 2025.
...
\bibitem[37]{ref37} J. Yuan et al. Understanding and Mitigating Numerical Sources of Nondeterminism in LLM Inference. arXiv:2506.09501, 2025.
\bibitem[38]{ref38} Survey on Evaluation of LLM-based Agents. arXiv:2503.16416, 2025.
