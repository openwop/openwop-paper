...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{(C3, demonstrated)} \emph{Honesty by construction}: advertisement bound to live state +
        strict conformance fails dishonest claims. \TODO{scope to ``the agent-protocol setting'' per
        Target-A finding, or claim unqualified if the 3rd sweep clears it.}
...
\TODO{State the value-recovery formula in full so a reviewer can recompute it; report n and window;
separate measured from modeled; name confounds (selection bias, no control, blended-rate abstraction).
Bracket with the independent evidence: METR RCT [METR] ($-19\%$) and the Management Science 3-RCT [MSRCT]
($+26\%$). Per the provenance finding, either cite the exact (vendor) source for any ROI figure and
label it vendor-funded, or drop the figure.}
...
\TODO{Solo steward, single estate, AI-authored corpus, self-evaluation bias, ROI confounds, the
perception-reality gap demonstrated by [METR]. Honesty-by-construction novelty bounded to the
agent-protocol setting unless the adjacent-domain sweep clears it.}
...
\bibitem[MSRCT]{msrct} Z.~Cui, M.~Demirer, S.~Jaffe et al. ``The Effects of Generative AI on High-Skilled Work: Evidence from Three Field Experiments with Software Developers.'' Management Science, 2025. \emph{(peer-reviewed; vendor tool)}
\bibitem[MCKINSEY]{mckinsey} McKinsey. ``Rewiring software delivery for the agentic era.'' 2025. \emph{(vendor/consultancy)}
